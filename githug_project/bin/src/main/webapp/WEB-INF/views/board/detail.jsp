...
$(document).ready(function(){
	$("#deletebtn").on('click', function(){
		...
		if(userpw == dbpw){
			location.href = "boarddelete?seq=${detaildto.seq}";  //get방식만 전달 - 간단할때 사용 그렇지 않으면 아래 두줄로 대체
	...
	$("#updatebtn").on('click', function(){
		...
		let dbpw = $("input:hidden").val();
		if(userpw == dbpw){
			//location.href = "boardupdate?seq=${updatedto.seq}";
			$("form").attr("action", "boardupdate");  //get, post 방식 모두 가능
			$("form").submit();  //form 내부 모든 input name 변수이름 전달
		}
		else{
			alert("암호를 확인하세요");
		}
...
<form action="" method="">
	...
	<tr><td>내용</td><td><textarea name="contents" rows=5 cols=50>${detaildto.contents }</textarea></td></tr>
